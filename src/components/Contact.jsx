import React from 'react'

function Contact() {
  return (
    <div name="contact" className='w-full h-screen bg-[#0a192f] flex justify-center items-center'>
        <form method='POST' action='' className='flex flex-col max-w-[600px] w-full '>
            <div className='pb-8'>
                <p className='text-4xl font-bold inline border-b-4 border-[#66ff87] text-gray-300'>Contacto</p>
                <p className='text-gray-300 py-4'>Porfavor completa el formulario o mandame un email - santiaviles17@gmail.com</p>
            </div>
            <input className='bg-[#ccd6f6] p-2' type='text' placeholder='Nombre' name='name'/>
            <input className='my-4 p-2 bg-[#ccd6f6]' type='text' placeholder='Email' name='email'/>
            <textarea className='bg-[#ccd6f6] p-2' name='message' rows='10' placeholder='Mensaje'/>
            <button className='text-white border-2  hover:border-[#66ff87] hover:text-[#66ff87] px-4 py-3 my-8 mx-auto flex items-center'>Colaboremos!</button>
        </form>
    </div>
  )
}

export default Contact